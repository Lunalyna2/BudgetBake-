...
export default function CostCalculator() {
  ...
  const [ingredients, setIngredients] =
    useState<Ingredient[]>(ingredientTemplates);
  ...
  useEffect(() => {
    const fetchRecipeData = async () => {
      if (!selectedRecipeId) {
        setIngredients(ingredientTemplates);
      ...
      try {
        // Selecting a recipe means the user must press
        // Scale Batch again before Generate Summary.
        setBatchScaled(false);
        setSummaryGenerated(false);
        setIsSummaryModalOpen(false);
        ...
        if (!calculationResponse.ok) {
          throw new Error(
            "Failed to fetch saved cost calculation.",
          );
        }
        ...
        // IMPORTANT:
        // A saved calculation does NOT count as pressing
        // Scale Batch during the current session.
        setBatchScaled(false);
        setSummaryGenerated(false);
        setIsSummaryModalOpen(false);
      } catch (error) {
          ...
          <div className="w-full max-w-2xl rounded-[28px] bg-[#fffdfd] p-6 shadow-[0_24px_80px_rgba(141,75,103,0.22)]">
            ...
            <div className="mt-5 rounded-[22px] border border-[#f3d9e5] bg-[#fff8fb] p-5">
              <div className="flex flex-col gap-4 border-b border-dashed border-[#f2d8e3] pb-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <h3 className="text-[1.25rem] font-black text-[#5d4a52]">
                    {recipeName}
                  </h3>
                ...
                <div className="rounded-2xl bg-white px-4 py-3 text-right shadow-sm">
                  <p className="text-xs font-bold uppercase tracking-[0.08em] text-[#c96f96]">
                    Suggested Price
                  </p>

                  <p className="text-2xl font-black text-[#e64d8d]">
                    {currency.format(
                      totals.sellingPricePerCookie,
                    )}
                  </p>

                  <p className="text-xs font-semibold text-[#8f6b79]">
                    per cookie
                  </p>
                </div>
              </div>
              ...
              <div className="mt-5 space-y-3 text-[0.95rem]">
                ...
                <div className="flex items-center justify-between rounded-2xl bg-[#f6fff2] px-4 py-3 shadow-sm">
                  <span className="font-semibold text-[#4f6a3f]">
                    Estimated Profit
                  </span>
