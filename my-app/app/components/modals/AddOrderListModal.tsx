...
export default function AddOrderListModal({
  ...
  useEffect(() => {
    ...
    const loadRecipes = async () => {
      try {
        ...
        const loadedRecipes: RecipeOption[] = (
          result.recipes ?? []
        ).map((recipe: any) => ({
        ...
        if (loadedRecipes.length > 0) {
          setSelectedRecipeId(
            loadedRecipes[0].recipe_id
          );
        }
      } catch (error) {
        console.error(
          "Failed to load recipes for order list:",
          error
        );
  ...
  const orderName = selectedRecipe?.name ?? "";
      ...
      >
        ...
        <div className="mt-5 space-y-5">
          <div>
            ...
            <select
              ...
              className="w-full rounded-2xl border border-pink-100 bg-[#FFF7FB] px-4 py-3 text-sm font-semibold text-[#5A0D36] focus:border-[#D291BC] focus:outline-none"
            >
              {recipes.length === 0 ? (
                <option value="">
            ...
            {recipes.length === 0 && (
              <p className="mt-2 text-xs text-red-500">
                Create a recipe in Recipe Library
                first.
              </p>
            )}
          </div>
            ...
            <div className="space-y-3">
              {customerRows.map(
                (row, index) => (
                  <div
                    key={row.id}
                    className="flex items-center gap-3 rounded-2xl border border-pink-100 bg-[#FFF7FB] p-3"
                  >
                    <input
                      type="text"
        ...
        <div className="mt-6 flex justify-end gap-3 border-t border-pink-100 pt-4">
          ...
          <button
            ...
            className="rounded-full bg-gradient-to-r from-[#B185DB] via-[#D291BC] to-[#FFC3D0] px-6 py-2 text-xs font-bold uppercase tracking-[0.2em] text-white shadow-md transition hover:opacity-95 disabled:cursor-not-allowed disabled:opacity-50"
          >
            Enter
          </button>
        </div>
