...
export default function RemindersCard({
  ...
  return (
    <div className="relative flex flex-col justify-between rounded-3xl border border-gray-200/80 bg-white p-8 shadow-sm">
      <div>
        <div className="flex items-center justify-between">
          <h2 className="text-3xl font-bold text-[#5A0D36]">Reminders</h2>
          <button
            type="button"
            onClick={onOpenModal}
            className="rounded-full bg-pink-100/70 p-2.5 text-[#E94E77] hover:bg-pink-100 transition-colors"
        ...
        <div className="mt-4 space-y-3">
          {reminders.slice(0, 3).map((reminder) => (
            <label
              key={reminder.id}
              className="flex cursor-pointer items-center gap-3 text-sm font-medium text-amber-900/80"
            >
              <input
                type="checkbox"
